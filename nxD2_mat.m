% nxD2 = nxD4_mat(obj, obj2, trian_par)
% nxD2 = <w_m,f_n> scalar producs of testing nxRWG (original mesh) with basis RWG (refined mesh)
% Integration on testing functions: 3 Gauss points (on the smaller triangle -> refined mesh)
%
% w_m = nxRWG, f_n = RWG_refined
%
% Input: 
...
function nxD2 = nxD4_mat(obj, obj2, trian_pr)
...
for T2=1:Nt2  % Basis triangle, that belongs up to 3 refined basis functions
    ...
    % Parent triangle in original mesh, and nxRWGs sharing this triangle
    T = trian_pr(T2, 1); % Parent triangle, of testing original nxRWG
	...
	nxD2(ge,ge2) = nxD2(ge,ge2) + (obj.ln(ge).'*obj2.ln(ge2)) .* tmp / (2*obj.ds(T));
end
