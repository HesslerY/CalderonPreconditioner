% [J_cl, J_test] = j_ex_test(a,D,obj,k,eta)
% Exact current for spheres 
% Expressed as linear combination of RWG basis functions and tested with RWG
...
%
% Output:
% J_cl = Jexact as linear combination of RWG
% J_test = Jexact tested with RWG
%
% Input:
% a	= sphere radius
...
function [J_cl, J_test] = j_ex_test(a,D,obj,k,eta)
...
%	Pn1=-Pn(2,:);					% Legendre function degree n order 1 <11-10-96
   ...
   Pn1sinth=(Pn1./sinth);				% Legendre function degree n order 1
	...
	an=(1j)^(-n)*(2*n+1)/(n*(n+1));		% n-coefficient

	termth = an*(((sinthPn1d)/H2nd) + 1j*(Pn1sinth/H2n));
	termph = an*((Pn1sinth/H2nd) + 1j*((sinthPn1d)/H2n));
	Jth = Jth + termth;
	Jph = Jph + termph;
...
Jth=1j*cosph.*Jth/(eta*k*a);
Jph=1j*sinph.*Jph/(eta*k*a);
...
J_test = (-ln.*sum((Jla(:,Tp).*rp)+(Jla(:,Tm).*rm))/2)'; % Test current with RWG
